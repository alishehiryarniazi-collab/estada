    ...
    <div className="flex min-h-screen flex-col bg-canvas">
      ...
      <main className="mx-auto w-full max-w-7xl flex-1 px-4 py-8 sm:px-6">
        <div className="mb-6 flex items-center justify-between">
          <h1 className="font-heading text-2xl font-semibold text-ink">{t('short.title')}</h1>
          {items.length > 0 && (
            <button
              onClick={share}
            ...
            >
              ...
              {copied ? t('short.linkCopied') : t('short.share')}
            </button>
          )}
        </div>
        ...
        {!user ? (
          <div className="rounded-card border border-hairline bg-surface p-10 text-center">
            ...
            <p className="mt-3 text-ink">{t('short.loginPrompt')}</p>
            <button
